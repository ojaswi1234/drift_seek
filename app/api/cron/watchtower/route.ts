  ...
  try {
    ...
    for (const server of servers) {
      ...
      await redis.hset(statsKey, {
        status, latency, reason, lastChecked: new Date().toISOString()
      });
      ...
      if (status === 'down') {
        ...
        if (!alreadyAlerted) {
          console.log(`[ALERT] ${server.url} is down. Emailing ${server.ownerEmail}`);
          
         
          await sendEmailAlert({
            to: server.ownerEmail,
            subject: `🚨 DRIFTSEEKER ALERT: ${server.url} is DOWN`,
